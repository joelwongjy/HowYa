...
class Region {
  ...
  init(filename: String) {
    guard
      let properties = Region.plist(filename) as? [String: Any],
      let boundaryPoints = properties["boundary"] as? [String]
      else { return }

    midCoordinate = Region.parseCoord(dict: properties, fieldName: "midCoord")
    overlayTopLeftCoordinate = Region.parseCoord(
      dict: properties,
      fieldName: "overlayTopLeftCoord")
    overlayTopRightCoordinate = Region.parseCoord(
      dict: properties,
      fieldName: "overlayTopRightCoord")
    overlayBottomLeftCoordinate = Region.parseCoord(
      dict: properties,
